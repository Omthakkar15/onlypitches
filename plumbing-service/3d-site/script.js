...
const roomData = [
  {
    ...
    "price": "From $120+",
    "image": "https://images.unsplash.com/photo-1585704032915-34077ca1e3d8?w=1400&q=85&sig=10"
  },
  {
    "title": "Water Heater",
    "desc": "Install included",
    "price": "From $800+",
    "image": "https://images.unsplash.com/photo-1585704032915-34077ca1e3d8?w=1400&q=85&sig=11"
  },
  {
    "title": "Repiping",
    "desc": "Whole home",
    "price": "From $2500+",
    "image": "https://images.unsplash.com/photo-1585704032915-34077ca1e3d8?w=1400&q=85&sig=12"
  }
];
...
function initHeroScene() {
  ...
  heroScene.add(dir);
  const point = new THREE.PointLight(0x3498db, 1.5, 20);
  ...
  const pMat = new THREE.PointsMaterial({ color: 0x3498db, size: 0.05, transparent: true, opacity: 0.65 });
...
function initAmbientScene() {
  ...
  ambientPoints = new THREE.Points(geo, new THREE.PointsMaterial({ color: 0x3498db, size: 0.06, transparent: true, opacity: 0.28 }));
...
function buildAmenityIcon(type) {
  ...
  const goldGlow = goldMaterial(0x3498db, 0.25);
...
function initRoomViewer() {
  ...
  roomScene.add(light);
  const rim = new THREE.DirectionalLight(0x3498db, 0.35);
...
function initAmenities3D() {
  ...
  canvases.forEach((canvas) => {
    ...
    scene.add(dl);
    const accent = new THREE.PointLight(0x3498db, 0.5, 8);
