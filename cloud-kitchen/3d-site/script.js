...
const roomData = [
  {
    "title": "Smash Burger",
    "desc": "Double patty, secret sauce",
    "price": "From $14",
    "image": "https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=1400&q=85&sig=10"
  },
  {
    "title": "Buddha Bowl",
    "desc": "Quinoa, roasted veg",
    "price": "From $13",
    "image": "https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=1400&q=85&sig=11"
  },
  {
    "title": "Miso Ramen",
    "desc": "Rich broth, soft egg",
    "price": "From $16",
    "image": "https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=1400&q=85&sig=12"
  }
];
...
function initHeroScene() {
  ...
  heroScene.add(dir);
  const point = new THREE.PointLight(0x00d9ff, 1.5, 20);
  ...
  const pMat = new THREE.PointsMaterial({ color: 0x00d9ff, size: 0.05, transparent: true, opacity: 0.65 });
...
function initAmbientScene() {
  ...
  ambientPoints = new THREE.Points(geo, new THREE.PointsMaterial({ color: 0x00d9ff, size: 0.06, transparent: true, opacity: 0.28 }));
...
function buildAmenityIcon(type) {
  ...
  const goldGlow = goldMaterial(0x00d9ff, 0.25);
...
function initRoomViewer() {
  ...
  roomScene.add(light);
  const rim = new THREE.DirectionalLight(0x00d9ff, 0.35);
...
function initAmenities3D() {
  ...
  canvases.forEach((canvas) => {
    ...
    scene.add(dl);
    const accent = new THREE.PointLight(0x00d9ff, 0.5, 8);
