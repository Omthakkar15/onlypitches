...
function initHeroScene() {
  ...
  const pMat = new THREE.PointsMaterial({ color: 0xf39c12, size: 0.05, transparent: true, opacity: 0.65 });
...
function initAmbientScene() {
  ...
  ambientPoints = new THREE.Points(geo, new THREE.PointsMaterial({ color: 0xf39c12, size: 0.06, transparent: true, opacity: 0.28 }));
...
function buildAmenityIcon(type) {
  ...
  const goldGlow = goldMaterial(0xf39c12, 0.25);
...
function initRoomViewer() {
  ...
  roomScene.add(light);
  const rim = new THREE.DirectionalLight(0xf39c12, 0.35);
...
function initAmenities3D() {
  ...
  canvases.forEach((canvas) => {
    ...
    scene.add(dl);
    const accent = new THREE.PointLight(0xf39c12, 0.5, 8);
